...
public class ClientesController implements Initializable {
    ...
    @FXML
    void cpf_mascara(KeyEvent event) {
        TextFieldFormatter tff = new TextFieldFormatter();
        tff.setMask("###.###.###-##");
        tff.setCaracteresValidos("0123456789");
        tff.setTf(cpf);
    ...
    void cadastrar(MouseEvent event) throws IOException {
        ...
        Clientes cliente = new Clientes(cpf.getText(), nome.getText(), email.getText(), endereco.getText(), numero.getText(), complemento.getText(), bairro.getText(), cidade.getText(), telefone_fixo.getText(), telefone_celular.getText(), data_cadastro.getText());
        ...
        if (errorMessage.length() == 0) {
            ...
            dao.ValidarClientes(cliente);
            if (dao.resultado) {
                alert.setTitle("Alerta");
                alert.setHeaderText("Não foi possível realizar o cadastro!");
                alert.setContentText("Cliente já existente!");
                alert.showAndWait();
            } else {
                dao.InserirClientes(cliente);
    ...
    @FXML
    private void adicionar_clientes(MouseEvent event) throws IOException {
        Parent fxml = FXMLLoader.load(getClass().getResource("/menu/Clientes.fxml"));
        anchor_pane1.getChildren().removeAll();
        anchor_pane1.getChildren().setAll(fxml);  
    }
    
    @FXML
    private void tabela_clientes(MouseEvent event) throws IOException {
        Parent fxml = FXMLLoader.load(getClass().getResource("/menu/TabelaClientes.fxml"));
        area_cadastro1.getChildren().removeAll();
        area_cadastro1.getChildren().setAll(fxml);  
    }
